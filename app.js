...
function descriptionLine(pair){let html='';if(pair.a1)html+=`A1 · ${esc(pair.a1.code)}${pair.inventoryA1?` (${esc(pair.inventoryA1)})`:''}`;if(pair.a2){if(html)html+='<br>';html+=`A2 · ${esc(pair.a2.code)}${pair.inventoryA2?` (${esc(pair.inventoryA2)})`:''}`}if(pair.qtn)html+=`<br>QTN · ${esc(pair.qtn)}`;if(pair.boq)html+=`<br>BOQ · ${esc(pair.boq)}`;return html||'未指定產品'}
...
<div class="p-inner-tabs"><button class="p-inner-tab active" data-ptab="${p.id}" data-ptab-panel="info">基本資料</button><button class="p-inner-tab" data-ptab="${p.id}" data-ptab-panel="items">需求資訊</button><button class="p-inner-tab" data-ptab="${p.id}" data-ptab-panel="notes">QS 備註</button></div>
...
<div class="p-inner-panel" data-ptab-panel="${p.id}|notes" style="display:none"><form class="qs-notes-form" data-qs-notes="${p.id}"><label>QS 備註<textarea name="qsNotes" placeholder="輸入 QS 相關備註...">${esc(p.qsNotes||'')}</textarea></label><button class="primary" type="submit">儲存備註</button></form></div>
...
// QS 備註 form handler
document.addEventListener('submit',e=>{const form=e.target.closest('[data-qs-notes]');if(!form)return;e.preventDefault();const p=state.projects.find(x=>x.id===form.dataset.qsNotes);if(!p)return;p.qsNotes=new FormData(form).get('qsNotes')||'';save();toast('QS 備註已儲存')});
document.addEventListener('click',e=>{const btn=e.target.closest('[data-ptab]');if(!btn)return;const pid=btn.dataset.ptab;document.querySelectorAll(`[data-ptab="${pid}"]`).forEach(t=>t.classList.remove('active'));btn.classList.add('active');const panel=btn.dataset.ptabPanel;document.querySelectorAll(`[data-ptab-panel="${pid}|info"],[data-ptab-panel="${pid}|items"],[data-ptab-panel="${pid}|notes"]`).forEach(p=>{p.style.display=p.dataset.ptabPanel===`${pid}|${panel}`?'':'none'});});
