...
document.addEventListener('submit',e=>{const form=e.target.closest('[data-worklog]');if(!form)return;e.preventDefault();const p=state.projects.find(x=>x.id===form.dataset.worklog);if(!p)return;const fd=new FormData(form);const names=['wl1','wl2','wl3','wl4','wl5','wl6'];const wl={};names.forEach(n=>wl[n]=fd.get(n)||'-');const summary=names.map(n=>wl[n]).filter(v=>v&&v!=='-').join('');if(!summary){toast('請至少選擇一個非「-」的 Work Log');return}p.workLog=wl;if(!Array.isArray(p.workLogs))p.workLogs=[];const qtnNum=(fd.get('qtnNum')||'').trim();p.workLogs.unshift({id:id(),summary,qtnNum,status:'submited',createdAt:new Date().toISOString()});save();renderProjects();toast('Log 已新增: '+summary)});
