...
function saveMix(){ localStorage.setItem('t1-mixmatch', JSON.stringify(mixState)); var _fsm = (window.T1||{}).firestore; if (_fsm && _fsm.ready && _fsm.saveMixState) _fsm.saveMixState(mixState); }
...
document.addEventListener('change',e=>{const sel=e.target.closest('[data-matched-classify]');if(!sel)return;const [projectId,itemId]=sel.dataset.matchedClassify.split('|');const newType=sel.value;if(!newType){return}const p=state.projects.find(x=>x.id===projectId);if(!p)return;const item=p.items.find(x=>x.id===itemId);if(!item)return;item.type=newType;item.extra={};const _a=item.pair||{};const _a1=_a.a1,_a2=_a.a2;const _label=o=>o?(o.code?o.code+' ':'')+(o.name||''):'';if(newType==='PARTITION'){if(_a1)item.extra.verticalSection=_label(_a1);if(_a2)item.extra.horizontalSection=_label(_a2)}else if(newType==='DOOR'){if(_a1)item.extra.doorFrame=_label(_a1);if(_a2)item.extra.doorPanel=_label(_a2)}save();renderProjects();const _panel=document.querySelector(`.p-inner-tab[data-ptab="${projectId}"][data-ptab-panel="${newType.toLowerCase()}"]`);if(_panel)_panel.click();toast('已分類為 '+newType)});
...
// === ACCESS MANAGEMENT — flow groups（取代 RFQ Wiki 區塊 UI）===
var flowGroups = [];
try { flowGroups = JSON.parse(localStorage.getItem('t1-access-mgmt') || '[]'); } catch(e) { flowGroups = []; }
function accessMgmtSave() {
  localStorage.setItem('t1-access-mgmt', JSON.stringify(flowGroups));
  var _fsa = (window.T1 || {}).firestore;
  if (_fsa && _fsa.ready && _fsa.saveAccessMgmt) _fsa.saveAccessMgmt(flowGroups); // 雲端備份（跨設備）
}
function renderAccessMgmt() {
  var panel = document.getElementById('wikiContent');
  if (!panel) return;
  var list = flowGroups.length ? flowGroups.map(function(g) {
    return '<div class="access-flow" data-flow="' + g.id + '"><div class="access-flow-head"><strong>' + esc(g.name) + '</strong>' + (g.endOfFlow ? '<span class="access-eof-badge">🏁 END OF FLOW</span>' : '') + '</div>' +
      (g.link ? '<div class="access-flow-row"><label>SHAREPOINT QUICK ACCESS LINK</label><a href="' + esc(g.link) + '" target="_blank" rel="noopener">' + esc(g.link) + '</a></div>' : '') +
      (g.description ? '<div class="access-flow-row"><label>DESCRIPTION TITLE</label><span>' + esc(g.description) + '</span></div>' : '') +
      (g.subTask ? '<div class="access-flow-row"><label>SUB TASK</label><span>' + esc(g.subTask) + '</span></div>' : '') +
      (g.refFolder ? '<div class="access-flow-row"><label>REFERENCE FOLDER</label><span>' + esc(g.refFolder) + '</span></div>' : '') +
      '<div class="access-flow-actions"><button type="button" class="access-eof" data-flow-end="' + g.id + '">END OF FLOW</button><button type="button" class="worklog-del" data-flow-del="' + g.id + '" title="刪除">✕</button></div></div>';
  }).join('') : '<div class="access-empty">尚未建立任何 Flow Group。點擊 ADD FLOW GROUP 開始。</div>';
  panel.innerHTML = '<div class="access-mgmt"><div class="access-mgmt-head"><h3>ACCESS MANAGEMENT</h3><button type="button" class="primary access-add" id="accessAddBtn">ADD FLOW GROUP</button></div>' +
    '<form class="access-form" id="accessForm" style="display:none"><div class="access-field"><label>NAME YOUR FLOW</label><input name="name" placeholder="e.g. RFQ Submission" /></div>' +
    '<div class="access-field"><label>SAVE SHAREPOINT QUICK ACCESS LINK</label><input name="link" type="url" placeholder="https://..." /></div>' +
    '<div class="access-field"><label>DESCRIPTION TITLE</label><input name="description" placeholder="Title / description" /></div>' +
    '<div class="access-field"><label>SUB TASK</label><input name="subTask" placeholder="Sub task" /></div>' +
    '<div class="access-field"><label>REFERENCE FOLDER</label><input name="refFolder" placeholder="Folder path" /></div>' +
    '<button type="submit" class="primary access-eof-submit">END OF FLOW</button></form>' +
    '<div class="access-list">' + list + '</div></div>';
  var addBtn = document.getElementById('accessAddBtn');
  if (addBtn) addBtn.onclick = function() { var f = document.getElementById('accessForm'); f.style.display = f.style.display === 'none' ? '' : 'none'; };
  var form = document.getElementById('accessForm');
  if (form) form.onsubmit = function(e) {
    e.preventDefault();
    var fd = new FormData(form);
    var name = (fd.get('name') || '').trim();
    if (!name) { toast('請填寫 NAME YOUR FLOW'); return; }
    flowGroups.unshift({ id: id(), name: name, link: (fd.get('link') || '').trim(), description: (fd.get('description') || '').trim(), subTask: (fd.get('subTask') || '').trim(), refFolder: (fd.get('refFolder') || '').trim(), endOfFlow: false, createdAt: new Date().toISOString() });
    accessMgmtSave(); renderAccessMgmt(); toast('Flow Group 已新增: ' + name);
  };
  panel.querySelectorAll('[data-flow-end]').forEach(function(b) {
    b.onclick = function() {
      var g = flowGroups.find(function(x) { return x.id === b.dataset.flowEnd; });
      if (g) { g.endOfFlow = !g.endOfFlow; accessMgmtSave(); renderAccessMgmt(); toast(g.endOfFlow ? '已標記 END OF FLOW' : '已取消 END OF FLOW'); }
    };
  });
  panel.querySelectorAll('[data-flow-del]').forEach(function(b) {
    b.onclick = function() {
      flowGroups = flowGroups.filter(function(x) { return x.id !== b.dataset.flowDel; });
      accessMgmtSave(); renderAccessMgmt(); toast('Flow Group 已刪除');
    };
  });
}
...
// Initialize when page loads — ACCESS MANAGEMENT replaces the RFQ Wiki tab UI
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(renderAccessMgmt, 600); });
} else {
  setTimeout(renderAccessMgmt, 600);
}
